...
public class ConfusionMatrix
{
    ...
    public double getWeightedPrecision()
    {
        ...
        for (String label : this.allGoldLabels) {
            sum += (this.getPrecisionForLabel(label) * this.getRowSum(label));
        }
        double wt_f_m = (sum / (double)this.getTotalSum());
        return wt_f_m;
    }
    
    /**
     * Computes Weighted Recall
     *
     * @return double
     */
    public double getWeightedRecall()
    {
        double sum = 0.0;
        for (String label : this.allGoldLabels) {
            sum += (this.getRecallForLabel(label) * this.getRowSum(label));
        }
        double wt_f_m = (sum / (double)this.getTotalSum());
        return wt_f_m;
    }
    ...
    public double getWeightedFMeasure()
    {
        double sum = 0.0;
        for (String label : this.allGoldLabels) {
            sum += (this.getFMeasureForLabels().get(label) * this.getRowSum(label));
        }
        double wt_f_m = (sum / (double)this.getTotalSum());
        return wt_f_m;
    }
